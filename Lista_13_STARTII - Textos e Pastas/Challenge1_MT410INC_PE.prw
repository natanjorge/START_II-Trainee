...
User Function MT410INC()
    ...
    Private cMsgLog := 'LOG DO PEDIDO DE VENDA!'+ CRLF + CRLF  
    Private cHoraGer := Time()

    cMsgLog += '[' + cHoraGer + ']' + 'Consulta no banco de dados realizada com sucesso!' + CRLF
...
Static Function MontaRel(cAlias)
    ...
    Local cCamPDF    := CriaPasta()//'C:\TOTVS\VsCode\Relatorios_L11\'
    Local cArqPDF    := AllTrim(C5_NUM)+'.pdf'

    cMsgLog += '[' + cHoraGer + ']' + 'Criação da pasta realizada com sucesso!' + CRLF
    ...
    cMsgLog += '[' + cHoraGer + ']' + 'Fontes definidas para o relatório são válidas!' + CRLF
...
Static Function Cabecalho(cAlias)
    ...
    cMsgLog += '[' + cHoraGer + ']' + 'Criação do cabeçalho realizada com sucesso!' + CRLF
...
Static Function ImpDados(cAlias)
    ...
    cMsgLog += '[' + cHoraGer + ']' + 'Impressão dos dados do cabeçalho realizada com sucesso!' + CRLF
...
Static Function ImpRodape(cAlias)
    ...
    cMsgLog += '[' + cHoraGer + ']' + 'Criação do rodapé realizada com sucesso!' + CRLF
    ...
    cMsgLog += '[' + cHoraGer + ']' + 'Impressão dos dados do rodapé realizada com sucesso!' + CRLF
...
Static Function VeriQuebLn(cString, nLineTam, nCol)
    ...
    cMsgLog += '[' + cHoraGer + ']' + 'Verificação da quebra de linha realizada com sucesso!' + CRLF
...
Return 'C:\TOTVS\Protheus\protheus_data\Pedidos de Venda\'


Static Function EscrTxt(cAlias) //?cgetfile para o usuário selecionar

  Local cPasta   := 'C:\TOTVS\Protheus\protheus_data\Pedidos de Venda\'               
  Local cArquivo := AllTrim(C5_NUM) + '.txt'                       
  Local oWriter := FWFileWriter():New(cPasta + cArquivo, .T.) 
    
    cMsgLog += '[' + cHoraGer + ']' + 'Geração do log realizada com sucesso!' + CRLF

  if File(cPasta + cArquivo) 
    FwAlertInfo('O arquivo já existe na pasta!', 'Atenção')
  else
    if !oWriter:Create() 
      FwAlertError('Houve um erro ao gerar o arquivo: ' + CRLF + oWriter:Error():Message, 'Atenção') //! Message é um errorlog da função Error    

    else 
      oWriter:Write(cMsgLog)

      oWriter:Close()       //? Fechando o arquivo
