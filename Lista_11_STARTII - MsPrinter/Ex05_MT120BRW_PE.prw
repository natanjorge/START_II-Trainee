...
User Function MT120BRW()   
	AAdd( aRotina, { 'Relatorio', "U_MSPrinter)", 0, 2 } )
Return
...
Static Function ImpDados(cAlias)
    ...
    Local cPedidoAnt := ''
    Local nPaginas   := 1, nValorTotal := 0//, nCont := 0
    ...
    while (cAlias)->(!EOF())
        ...
        if((cAlias)->(C7_NUM)<>cPedidoAnt)
            ...
            if nPaginas > 1
                ValTot(nLinha, nValorTotal)
                nValorTotal := 0
                VeriQuebPg(MAX_LINE)
            endif

            oPrint:Say(nLinha, 020, AllTrim((cALias)->(C7_NUM)),         oFont10,, PRETO)
            oPrint:Say(nLinha, 130, Transform(((cALias)->(C7_EMISSAO)), "@R 99/99/9999"), oFont10,, PRETO)
            oPrint:Say(nLinha, 250, AllTrim((cALias)->(C7_FORNECE)),     oFont10,, PRETO)
            oPrint:Say(nLinha, 390, AllTrim((cALias)->(C7_LOJA)),        oFont10,, PRETO)
            oPrint:Say(nLinha, 505, AllTrim((cALias)->(C7_COND)),        oFont10,, PRETO)

            nLinha += 60

            oPrint:Say(785, 550, AllTrim(STR(nPaginas)), oFont08,, PRETO)
            nPaginas++
...
Static Function ValTot(nLinha, nValorTotal)
